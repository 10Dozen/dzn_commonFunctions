...
/*
	...
	Params:
	0: _oneliner (ARRAY of chars) - oneliner line chars.
...
DBG_2("(parseOnelinerStructure) Oneliner: %1. Mode: %2", toString _oneliner, _mode);

private _chars = STRIP(_oneliner) + [ASCII_COMMA];
...
private _bracketsOpenCloseMap = _self get Q(BracketsOpenCloseMap);
...
for "_i" from 0 to _lenght do {
    ...
    switch _char do {
        ...
        case ASCII_COMMA: {
            ...
            DBG_1("(parseOnelinerStructure) Found item: %1", toString _item);

            switch _mode do {
                case ONELINER_ARRAY: {
                    DBG_1("(parseOnelinerStructure) Converting %1 to ARRAY item", toString _item);
                    private _value = _self call [F(parseValueType), [_item]];
                ...
                case ONELINER_HASHMAP: {
                    DBG_1("(parseOnelinerStructure) Converting %1 to HASHMAP key-value pair", toString _item);
                    private _parsed = _self call [F(parseKeyValuePair), [_item]];
        ...
        case ASCII_DOUBLE_QUOTE: {
            ...
            _inQuotes = !(_char == _openQuoteChar);
            _openQuoteChar = [-1, _openQuoteChar] select _inQuotes;
        ...
        case ASCII_SQUARE_BRACKET_OPEN: {
            ...
            if (!_inBracket) exitWith {
                ...
                _inBracket = true;
                _inBracketsDepth = 1; //_inBracketsDepth + 1;
                _openBracketChar = _char;
            };

            // Another nested structure found - just update depth counter
            _inBracketsDepth = _inBracketsDepth + ([0, 1] select (_char == _openBracketChar));
            /*
            if (_char == _openBracketChar) exitWith {
        ...
        case ASCII_SQUARE_BRACKET_CLOSE: {
            if (_inQuotes || !_inBracket) exitWith {}; // Ignore closing brackets inside the quotes or somewhere random

            DBG_1("(parseOnelinerStructure) [inBracket] Possible close bracket at pos %1.", _i);
            // In brackets and closing bracket is the same type as open one -- decrease depth. Once depth is 0 - exit in bracket mode.
            _inBracketsDepth = _inBracketsDepth - ([0,1] select (_char == (_bracketsOpenCloseMap get _openBracketChar)));
            _inBracket = _inBracketsDepth > 0;

            /*
            if (_char == (_bracketsOpenCloseMap get _openBracketChar)) exitWith {
                _inBracketsDepth = _inBracketsDepth - 1;

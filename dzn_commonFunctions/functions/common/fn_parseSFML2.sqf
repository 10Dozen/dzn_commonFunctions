...
params ["_input", ["_dataMode", MODE_FILE_LOAD], ["_args", []], ["_convertKeys", false]];
...
if (isNil "dzn_SFML") then {
            ...
            switch _char do {
                case ASCII_CR: {
                    if ([_char, _chars # (_i + 1)] isEqualTo _linebreak) then {
                        ...
                        _normalizedLine = toString (_self call ["removeComment", [_lineChars]]);
                ...
                case ASCII_NL: {
                    ...
                    _normalizedLine = toString(if (_isRawFile) then {
                        _self call ["removeComment", [_lineChars]]
                    } else {
                        _lineChars
                    });
                    _lines pushBack _normalizedLine;
    ...
    private _fnc_calculateExpectedIndent = {
        ...
        private _expected = 0;
        {
            _expected = _expected + ([INDENT_DEFAULT, INDENT_ARRAY_NESTED] select typename _x == "SCALAR");
            /*
                ...
                case ASCII_COMMA: {
                    ...
                    switch _mode do {
                        case ONELINER_ARRAY: {
                            ...
                            private _value = _self call ["parseValueType", [_itemStr]];
                        ...
                        case ONELINER_HASHMAP: {
                            ...
                            private _parsed = _self call ["parseKeyValuePair", [_itemStr]];
                            ...
                            _value = _self call ["parseValueType", [_value]];
    ...
    private _fnc_parseValueType = {
        ...
        if (_first == ARRAY_PREFIX && _last == ARRAY_POSTFIX) exitWith {
            ...
            (_self call ["parseOnelinerStructure", [_value, ONELINER_ARRAY]])
        };

        // HashMap case: (john: Doe, age: 33)
        if (_first == HASHMAP_PREFIX && _last == HASHMAP_POSTFIX) exitWith {
            DBG("(parseValueType) Value parsed to ONELINE HASHMAP.");
            (_self call ["parseOnelinerStructure", [_value, ONELINER_HASHMAP]])
        };
        ...
        _value = _self call ["removeEscaping", [_asChars]];
        (_value)
    };
    ...
    private _fnc_addSetting = {
        ...
        private _parsedValue = if (_parseType) then { _self call ["parseValueType", [_value]] } else { _value };
    ...
    private _fnc_findAndLinkRefValues = {
        ...
        */
        ...
        {
            _self call ["findRefValues", [_node, _x]];
    ...
    dzn_SFML = createHashMapObject [
        ["splitLines", _fnc_splitLines],
        ["removeComment", _fnc_removeComment],
        ["parseKeyValuePair", _fnc_parseKeyValuePair],
        ["addArrayItem", _fnc_addArrayItem],
        ["getNode", _fnc_getNode],
        ["addNode", _fnc_addNode],
        ["calculateExpectedIndent", _fnc_calculateExpectedIndent],
        ["initMultilineBuffer", _fnc_initMultilineBuffer],
        ["parseOnelinerStructure", _fnc_parseOnelinerStructure],
        ["checkIsOneliner", _fnc_checkIsOneliner],
        ["parseValueType", _fnc_parseValueType],
        ["addSetting", _fnc_addSetting],
        ["linkRefValues", _fnc_linkRefValue],
        ["findRefValues", _fnc_findRefValues],
        ["findAndLinkRefValues", _fnc_findAndLinkRefValues],
        ["convertToArray", _fnc_convertToArray],
        ["removeEscaping", _fnc_removeEscaping],
        ["parseLine", _fnc_parseLine]
    ];
};
...
private _lines = if (_dataMode == MODE_PARSE_LINE) then {
    ...
    dzn_SFML call ["splitLines", [_data]]
};
...
if (_dataMode == MODE_PARSE_LINE) then {
    ...
    _self call ["convertToArray", []];

    // Link reference values
    if (_hasReferences) then {
        _self call ["findAndLinkRefValues", [_hash]];
    };
};

...
switch (toLower(_this)) do {
	case "random": {
		private _wthr = selectRandom [0,0,0, .1,.1,.1 ,.2,.2 ,.3 ,.5 ,.7 ,.75 ,1];
		0 setOvercast _wthr;
		0 setRain (if (_wthr > 0.7) then { selectRandom [.5, .5, .7, 1] } else { 0 });	
	};
	case "clear": { 	0 setOvercast 0; 				};
	case "cloudy": {	0 setOvercast .25; 				};
	case "overcast": {	0 setOvercast .5; 				};
	case "rain": {	0 setOvercast .75; 0 setRain 0.5; 	};

...
if (isNil "_callback" || _remoteExecOwner == 0) exitWith {
...
if (_callback # 0 == "RCE") then {
    _callback params ["", "_callbackCobName", "_callbackMethod", ["_callbackArgs", []]];
    _callbackArgs pushBack _result;
    [
        _callbackCobName,
        _callbackMethod,
        _callbackArgs
    ] remoteExec ["dzn_fnc_receiveRCE", _remoteExecOwner];
} else {
    _callback params ["", "_callbackFunction", ["_callbackArgs", []]];
    _callbackArgs pushBack _result;
    _callbackArgs remoteExec [_callbackFunction, _remoteExecOwner];
};

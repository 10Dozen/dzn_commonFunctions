...
#define TEST test_modes_parseline

LOG_TEST_START;
INIT_FAILED_STEPS_COUNTER;

// Mode: Load file
private _src = "Key: 1, Section: (Key:2), Array: [1,2,3]";
private _s = [_src, "PARSE_LINE"] call dzn_fnc_parseSFML;
...
if (isNil "_r" || !_r) then {
    FAIL_STEP;
    ERROR_ "(PARSE_LINE) Source is invalid. Actual: [%1], but expected [%2]", _s get SOURCE_NODE, _src _EOL;
};
...
if (isNil "_r" || !_r) then {
    FAIL_STEP;
    ERROR_ "(PARSE_LINE) Keys/values are invalid!" _EOL;
};
